...
export const GlareCard = ({
  ...
  const backgroundStyle = {
    "--step": "50%",
    "--foil-svg": `url("data:image/svg+xml,%3Csvg width='26' height='26' viewBox='0 0 26 26' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M2.99994 3.419C2.99994 3.419 21.6142 7.43646 22.7921 12.153C23.97 16.8695 3.41838 23.0306 3.41838 23.0306' stroke='rgba(255, 255, 255, 1)' stroke-width='5' stroke-miterlimit='3.86874' stroke-linecap='round' style='mix-blend-mode:darken'/%3E%3C/svg%3E")`,
    "--pattern": "var(--foil-svg) center/100% no-repeat",
    "--rainbow": "repeating-linear-gradient( 0deg, rgba(255, 255, 255, 1) calc(var(--step) * 1), rgba(217, 217, 217, 1) calc(var(--step) * 2), rgba(166, 166, 166, 1) calc(var(--step) * 3), rgba(127, 127, 127, 1) calc(var(--step) * 4), rgba(102, 102, 102, 1) calc(var(--step) * 5), rgba(77, 77, 77, 1) calc(var(--step) * 6), rgba(51, 51, 51, 1) calc(var(--step) * 7) ) 0% var(--bg-y)/200% 700% no-repeat",
    "--diagonal": "repeating-linear-gradient( 128deg, rgba(14, 21, 46, 1) 0%, rgba(153, 153, 153, 1) 3.8%, rgba(153, 153, 153, 1) 4.5%, rgba(153, 153, 153, 1) 5.2%, rgba(14, 21, 46, 1) 10%, rgba(14, 21, 46, 1) 12% ) var(--bg-x) var(--bg-y)/300% no-repeat",
    "--shade": "none",
  ...
  return (
    ...
    >
      <div className="h-full grid will-change-transform origin-center transition-transform duration-[var(--duration)] ease-[var(--easing)] delay-[var(--delay)] [transform:rotateY(var(--r-x))_rotateX(var(--r-y))] rounded-[var(--radius)] border border-slate-800 hover:[--opacity:0.6] hover:[--duration:200ms] hover:[--easing:linear] hover:filter-none overflow-hidden">
        <div className="w-full h-full grid [grid-area:1/1] mix-blend-soft-light [clip-path:inset(0_0_0_0_round_var(--radius))]" style={{zIndex: '100'}}>
          <div className={cn("h-full w-full bg-[#09090b]", className)}>
            {children}
          </div>
        </div>
        <div className="w-full h-full grid [grid-area:1/1] mix-blend-soft-light [clip-path:inset(0_0_1px_0_round_var(--radius))] opacity-[var(--opacity)] transition-opacity transition-background duration-[var(--duration)] ease-[var(--easing)] delay-[var(--delay)] will-change-background [background:radial-gradient(farthest-corner_circle_at_var(--m-x)_var(--m-y),_rgba(255,255,255,1)_10%,_rgba(255,255,255,0.65)_20%,_rgba(255,255,255,0)_90%)]" />
        <div
